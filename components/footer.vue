<template>
        <footer>
            <div id="footer-desktop">
            <div id="contact" class="footer-content">
                <h1 class="slogan">ännyone is<br> welcome!</h1>
                <h5 class="footer-text">Du möchtest eigene Ideen einbringen, uns Feedback geben, hast einen spannenden Kontakt für uns oder möchtest erfahren, wie es mit uns weiter geht? Dann melde dich doch einfach bei uns! <br>Getreu unserem Motto: <br ><b>«ännything is possible.»</b></h5>
                <ButtonIcon
                icon="smiley"
                href="mailto:anny.startup@gmail.com">
                Schreib uns!</ButtonIcon>
            </div>
            <div class="footer-links">   
                <div class="rechtliches">     
                    <NuxtLink class="btn-global" to="/datenschutz">Datenschutz</NuxtLink>  
                    <NuxtLink class="btn-global" to="/impressum">Impressum</NuxtLink>  
                </div>    
                <div class="some">
                    <NuxtLink class="btn-global" to="https://www.linkedin.com/company/anny-multifunctional-furniture/about/">LinkedIn</NuxtLink>                 
                    <NuxtLink class="btn-global" to="mailto:anny.startup@gmail.com">Mail</NuxtLink> 
                </div>
            </div>
        </div>
        <div id="footer-mobile">
            <div id="contact" class="footer-content">
                <h1 class="slogan">ännyone is<br> welcome!</h1>
                <h5 class="footer-text">Du möchtest eigene Ideen einbringen, uns Feedback geben, hast einen spannenden Kontakt für uns oder möchtest erfahren, wie es mit uns weiter geht? Dann melde dich doch einfach bei uns! Getreu unserem Motto: <b class="highlight-text">«ännything is possible.»</b></h5>
                <ButtonIcon
                icon="smiley"
                href="mailto:anny.startup@gmail.com">
                Schreib uns!</ButtonIcon>
            </div>
            <div class="footer-links">   
                <div class="rechtliches">     
                    <NuxtLink class="btn-global" to="/datenschutz">Datenschutz</NuxtLink>  
                    <NuxtLink class="btn-global" to="/impressum">Impressum</NuxtLink>  
                </div>    
                <div class="some">  
                    <NuxtLink class="btn-global" to="https://www.linkedin.com/company/anny-multifunctional-furniture/about/">LinkedIn</NuxtLink>                 
                    <NuxtLink class="btn-global" to="mailto:anny.startup@gmail.com">Mail</NuxtLink> 
                </div>
            </div>  
        </div>
        </footer>
</template>